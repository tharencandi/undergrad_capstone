...
FONT_PATH = "/home/lab_server_04/cell_processing/glioblastoma_processing/data/Courier New Bold.ttf"
SUMMARY_TITLE_FONT_PATH = "/home/lab_server_04/cell_processing/glioblastoma_processing/data/Courier New Bold.ttf"
...
# def generate_top_tile_summaries_tp(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
#                                 label_all_tiles=LABEL_ALL_TILES_IN_TOP_TILE_SUMMARY,
#                                 border_all_tiles=BORDER_ALL_TILES_IN_TOP_TILE_SUMMARY):
#   """
#   Generate summary images/thumbnails showing the top tiles ranked by score.
...
def save_tile_summary_on_original_image_name(pil_img, image_name):
  """
  Save a tile summary on original image and thumbnail to the file system.
  ...
  """
  t = Time()
  filepath = slide.get_tile_summary_on_original_image_path_name(image_name)
  pil_img.save(filepath)
  print("%-20s | Time: %-14s  Name: %s" % ("Save Tile Sum Orig", str(t.elapsed()), filepath))
...
# def save_above_threshold(slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True, threshold = 0, save=False):
#   """
#   Generate tile summary and top tiles for slide.
...
#           x.save_tile_threshold(keep=True)
#       else:
#           #Update name
#           # x.__str__ = x.__str__ + "_delete"
#           x.save_tile_threshold(keep=False)
#   # else:
...
#   if save_data:
#     save_tile_data(tile_sum)
#   generate_tile_summaries_name(tile_sum, np_img, display=display, save_summary=save_summary)
#   generate_top_tile_summaries(tile_sum, np_img, display=display, save_summary=save_summary)
# #   if save_top_tiles:
...
# def image_list_to_tiles_threshold(image_num_list, display=False, save_summary=True, save_data=True, save_top_tiles=True, threshold = 0, save=False):
#   """
#   Generate tile summaries and tiles for a list of images.
...
#   tile_summaries_dict = dict()
#   for slide_num in image_num_list:
#     tile_summary = save_above_threshold(slide_num, display, save_summary, save_data, save_top_tiles, threshold, save)
#     tile_summaries_dict[slide_num] = tile_summary
#   return image_num_list, tile_summaries_dict


# def image_range_to_tiles_threshold(start_ind, end_ind, display=False, save_summary=True, save_data=True, save_top_tiles=True, threshold = 0, save=False):
#   """
...
# def singleprocess_filtered_images_to_tiles_(display=False, save_summary=True, save_data=True, save_top_tiles=True,
#                                            html=True, image_num_list=None, threshold=0):
#   """
#   Generate tile summaries and tiles for training images using a single process.
...
#   if image_num_list is not None:
#     image_num_list, tile_summaries_dict = image_list_to_tiles_threshold(image_num_list, display, save_summary, save_data,
#                                                               save_top_tiles,threshold=0)
#   else:
#     num_training_slides = slide.get_num_training_slides()
#     image_num_list, tile_summaries_dict = image_range_to_tiles_threshold(1, num_training_slides, display, save_summary, save_data,
#                                                                save_top_tiles, threshold=0)
...
# def multiprocess_filtered_images_to_tiles_threshold_png(display=False, save_summary=True, save_data=True, save_top_tiles=True,
#                                           html=True, image_num_list=None, threshold = 0, save=False):
#   """
#   Generate tile summaries and tiles for all training images using multiple processes (one process per core).
...
#   print("Filtering all images with cell coverage < " + str(threshold))
#   timer = Time()
#   print("Generating tile summaries (multiprocess)\n")
...
#     if image_num_list is not None:
#       sublist = image_num_list[start_index - 1:end_index]
#       tasks.append((sublist, display, save_summary, save_data, save_top_tiles, threshold,save))
#       print("Task #" + str(num_process) + ": Process slides " + str(sublist))
#     else:
#       tasks.append((start_index, end_index, display, save_summary, save_data, save_top_tiles, threshold,save))
#       if start_index == end_index:
#         print("Task #" + str(num_process) + ": Process slide " + str(start_index))
#       else:
#         print("Task #" + str(num_process) + ": Process slides " + str(start_index) + " to " + str(end_index))
...
#   for t in tasks:
#     if image_num_list is not None:
#       results.append(pool.apply_async(image_list_to_tiles_threshold, t))
#     else:
#       results.append(pool.apply_async(image_range_to_tiles_threshold, t))
...
#     image_nums, tile_summaries = result.get()
#     slide_nums.extend(image_nums)
#     summary_list.append(tile_summaries)
#     tile_summaries_dict.update(tile_summaries)
#     print("Done tiling slides: %s" % image_nums)
...
#   print("Time to generate tile previews (multiprocess): %s\n" % str(timer.elapsed()))
#   return tile_summaries_dict



# def multiprocess_filtered_images_to_tiles_threshold_svs(display=False, save_summary=True, save_data=True, save_top_tiles=True,
#                                           html=True, image_num_list=None, threshold = 0, save=False):
#   """
#   Generate tile summaries and tiles for all training images using multiple processes (one process per core).
