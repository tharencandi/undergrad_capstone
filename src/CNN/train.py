...
def grid_search(max_epochs, 
    ...
    for b in batch_sizes:
        ...
        dic_model_history["b{}e{}".format(b, max_epochs)] = model_history.history
...
def accuracy_plot(axis, history,title, save=False, show=True, single_use=False):
    ...
    history: History.history object generated by .fit()
...
def loss_plot(axis, history, title, save=False, show=True, single_use=False):
    ...
    if single_use:
        ...
        axis.plot(history['loss'])
        axis.plot(history['val_loss'])
        axis.set_title(title)
        axis.ylabel('loss')
        axis.xlabel('epoch')
        axis.legend(['train', 'val'], loc='upper left')
...
def main():
    ...
    if FUNC == functions.SINGLE_TRAIN:
        ...
        best, log, hist_dict = grid_search(1, 
                                            [32,40], 
                                            train, 
                                            val, 
                                            test, 
